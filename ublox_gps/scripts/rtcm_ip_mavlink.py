...
def read_rtcm():
  rtcm_ip = rospy.get_param("~rtcm_ip", "localhost")
  rtcm_port = rospy.get_param("~rtcm_port", 6666)
  rospy.loginfo("opening rtcm TCP port: " + rtcm_ip + " on baud %d" % (rtcm_port))
  ...
  try:
    sock.connect((rtcm_ip, rtcm_port))
    sock.settimeout(.1)
    rospy.loginfo('RTCM TCP Successful Connection')
  except:
    rospy.logwarn('RTCM TCP Connection Failed')
    sys.exit(0)
...
if __name__ == '__main__':
    ...
    address = rospy.get_param('~address', 'localhost')
